...
class gameMap(object):
    ...
    def mapMain(self,userIn):
        self.roomDesc(self,position[0],position[1])
        global flag
        userIn = userIn[0:1]
        self.checkDirect(self,userIn)
        if flag == True:
            QSO.quitGameGame()

    def room(self,x,y,direc):
        global flag
        position = [x,y]
        if position == [2,0]:
            #Exit
    ...
    def checkDirect(self,userIn):
        global position
        blockFlag = False
        if userIn == "n":
            if self.room(position[0],position[1]+1,userIn):
                position[1] += 1
            else:
                blockFlag = True
        elif userIn =="s":
            ...
            if self.room(position[0]+1,position[1],userIn):
                position[0] += 1
            else:
                blockFlag = True
        elif userIn =="w":
